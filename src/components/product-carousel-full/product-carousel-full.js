...
window.ComponentLoader.registerComponent('product-carousel-full', (root) => {
    ...
    const updateDescription = () => {
        ...
        if (sizesEl) {
            if (data.sizes.length > 0) {
                sizesEl.innerHTML = data.sizes
                    .map(size => `<span class="inline-block px-3 py-1.5 bg-gradient-to-r from-[#9aad7a] to-[#7a8d5a] text-white text-xs font-semibold rounded-full shadow-sm">${size}</span>`)
                    .join('');
            } else {
        ...
        // Actualizar features
        if (featuresEl) {
            featuresEl.innerHTML = data.features.length > 0
    ...
    root.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.js-add-to-cart')) {
            ...
            // Emit custom event
            root.dispatchEvent(new CustomEvent('product:added-to-cart', {
                detail: { productId },
                bubbles: true
            }));
        }
