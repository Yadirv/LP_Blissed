...
async function testSPAPI() {
  ...
  try {
    ...
    console.log(
      `   ${colors.cyan}Access Key:${colors.reset} ${stsResponse.Credentials.AccessKeyId}`
    );
    console.log(
    ...
    console.log(
      `   ${colors.yellow}Marketplace:${colors.reset} ${process.env.MARKETPLACE_ID} (USA)`
    );
    console.log(
      `   ${colors.yellow}Modo:${colors.reset} ${useSandbox ? "SANDBOX" : "PRODUCCIÓN"}\n`
    );
