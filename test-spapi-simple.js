...
async function testAuth() {
  ...
  try {
    ...
    console.log(
      `   ${colors.cyan}Temporal Access Key:${colors.reset} ${stsResponse.Credentials.AccessKeyId}`
    );
    console.log(
      `   ${colors.cyan}Expira:${colors.reset} ${stsResponse.Credentials.Expiration.toLocaleString("es-ES")}\n`
    );
    ...
    console.log(
      `   ${colors.cyan}Access Token (primeros 20 chars):${colors.reset} ${lwaData.access_token.substring(0, 20)}...`
    );
    console.log(`   ${colors.cyan}Token Type:${colors.reset} ${lwaData.token_type}`);
    console.log(
      `   ${colors.cyan}Expira en:${colors.reset} ${lwaData.expires_in} segundos (${lwaData.expires_in / 60} minutos)\n`
    );
